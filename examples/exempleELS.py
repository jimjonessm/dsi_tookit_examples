...
from dsi_toolkit import (generateCandidateTerms, removeClusters, detectStructure,
                 getInfo, estimateParametersELS, correlationFunction,
...
# the extended least squares estimator.
#

# Clear data, close open windows, and clear the command prompt
# --------------------------------------------------------------------------
# Equivalent to MATLAB's `clear all`, `close all`, and `clc`
system('cls')
close('all')
...
# Estimation using Extended Least Square
Parameters, ParametersP, ParametersR, vP, MP, e = estimateParametersELS(model, ui, yi, 100)

H, Y = correlationFunction(e, len(e), 1)
...
yest, eqest = displayStaticModel(staticModel, uest, y, 1)
